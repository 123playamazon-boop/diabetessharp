...
export function AdminPremiumPage() {
  ...
  const setPremium = async (suite: string, active: boolean) => {
    setBusySuite(suite);
    try {
      const res = await fetch(apiUrl(`/api/admin/clients/${encodeURIComponent(suite)}/premium`), {
        method: "POST",
      ...
      toast.success(active ? "Premium ativado." : "Premium revogado.");
      await refresh();
    } catch {
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        subtitle="Ative ou cancele o plano premium por suite. Os clientes veem o estado ao atualizar o perfil (login / página)."
        actions={
          <button
            type="button"
      ...
      <div className="overflow-hidden rounded-3xl border border-zinc-200/80 bg-white shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[720px] table-fixed border-collapse text-left text-[13px] leading-snug text-zinc-800">
            ...
            <tbody className="divide-y divide-zinc-100">
              {clients.length === 0 ? (
                ...
                clients.map((c) => {
                  const premium = c.premiumActive === true;
                  return (
                    <tr key={c.suite} className="hover:bg-zinc-50/80">
                      ...
                      <td className="px-4 py-3.5">
                        <span
                          className={
                            premium
                              ? "inline-flex rounded-full border border-violet-200 bg-violet-50 px-2.5 py-1 text-xs font-semibold text-violet-900"
                              : "inline-flex rounded-full border border-zinc-200 bg-white px-2.5 py-1 text-xs font-semibold text-zinc-600"
                          }
                        >
                          {premium ? "Premium ativo" : "Sem premium"}
                        </span>
                      </td>
                      <td className="px-4 py-3.5 text-right">
                        <div className="flex flex-wrap justify-end gap-2">
                          {!premium ? (
                            ...
                            >
                              {busySuite === c.suite ? "…" : "Cancelar premium"}
                            </button>
                          )}
                        </div>
                      </td>
